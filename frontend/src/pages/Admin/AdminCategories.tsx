...
const AdminCategories = () => {
  ...
  const canCreateCategory = can(role, "category", "create");
  const canUpdateCategory = can(role, "category", "update");
  const canDeleteCategory = can(role, "category", "delete");

  const canCreateBrand = can(role, "brand", "create");
  const canUpdateBrand = can(role, "brand", "update");
  const canDeleteBrand = can(role, "brand", "delete");
  ...
  const handleBrandSubmit = async () => {
    if (editingBrand && !canUpdateBrand) {
                  ...
                  <tbody>
                    {categories.map((category) => (
                      <tr key={category.id}>
                        ...
                        <td>
                          <span
                            className={`${styles.badge} ${category.isActive ? styles.badgeSuccess : styles.badgeDefault}`}
                          >
                  ...
                  <tbody>
                    {brands.map((brand) => (
                      <tr key={brand.id}>
                        ...
                        <td>
                          <span
                            className={`${styles.badge} ${brand.isActive ? styles.badgeSuccess : styles.badgeDefault}`}
                          >
        ...
        >
          <div className={styles.form}>
            ...
            <Input
              ...
              onChange={(e) =>
                setBrandForm({ ...brandForm, slug: slugify(e.target.value) })
              }
              error={formErrors.slug}
              placeholder="e.g., siemens"
            />
            ...
            <label className={styles.checkboxLabel}>
              <input
                ...
                onChange={(e) =>
                  setBrandForm({ ...brandForm, isAuthorized: e.target.checked })
                }
              />
