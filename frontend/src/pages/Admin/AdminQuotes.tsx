...
const AdminQuotes: React.FC = () => {
  ...
  const { isLoading: authLoading, isAuthenticated, admin } = useAdminAuth();
  const role = getAdminRoleFromUser(admin);
  const canUpdateQuote = can(role, "quote", "update");
  const canDeleteQuote = can(role, "quote", "delete");
  ...
  const fetchQuotes = useCallback(async () => {
    try {
      ...
      const errorMsg =
        _err instanceof Error ? _err.message : "Failed to fetch quotes";
      setError(errorMsg);
  ...
  const handleQuickStatusUpdate = async (
    ...
    if (!canUpdateQuote) {
      showToast("You do not have permission to update quotes", "error");
      return;
    }
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        _err instanceof Error ? _err.message : "Failed to update quote",
        "error",
      );
    } finally {
      setIsUpdating(false);
  ...
  const handleDelete = async (id: string, quoteName: string) => {
    if (!canDeleteQuote) {
      showToast("You do not have permission to delete quotes", "error");
      return;
    }
    if (
      !window.confirm(`Delete quote from ${quoteName}? This cannot be undone.`)
    ) {
      return;
    }
    ...
    <div className={styles.pageWrapper}>
      ...
      <div className="container">
        ...
        <div className={styles.resultsInfo}>
          Showing <strong>{filteredQuotes.length}</strong> of{" "}
              ...
              <div key={quote.id} className={styles.quoteCard}>
                ...
                <div className={styles.quoteBody}>
                  <div className={styles.customerInfo}>
                    ...
                    {quote.company && (
                      <p className={styles.companyName}>{quote.company}</p>
                    )}
                  </div>
                    ...
                    <p className={styles.messagePreview}>
                      ...
                      {quote.projectDetails && (
                        <>
                          ...
                          {quote.projectDetails.length > 80
                            ? `${quote.projectDetails.substring(0, 80)}...`
                            : quote.projectDetails}
                        </>
                      )}
                    </p>
            ...
            <div className={styles.modalSection}>
              ...
              <div className={styles.messageBox}>
                <div className={styles.infoRow}>
                  ...
                  <span className={styles.infoValue}>
                    {selectedQuote.productName || "Not specified"}
                  </span>
                </div>
                <div className={styles.infoRow}>
                  <span className={styles.infoLabel}>Quantity:</span>
                  <span className={styles.infoValue}>
                    {selectedQuote.quantity || "Not specified"}
                  </span>
                </div>
            ...
            <div className={styles.modalSection}>
              ...
              <div className={styles.formGroup}>
                ...
                <span className={styles.helperText}>
                  These notes are for internal use only and won't be visible to
                  customers
                </span>
              </div>
            </div>
