...
import { authLimiter, twoFactorLimiter } from '../../middlewares/rateLimit.middleware';
...
router.post('/login', authLimiter, validate(loginValidation), setCsrfToken, controller.login);
...
router.post('/verify-2fa', twoFactorLimiter, setCsrfToken, controller.verify2FA);
